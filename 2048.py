...
class Action(object):
    ...
    def get(self):
        input = "N"
        while input not in self.actions_dict:
            input = self.stdscr.getch()
        return self.actions_dict[input]
...
class Screen(object):
    ...
    def __init__(self, screen=None, grid=None, over=False, win=False):
        self.grid = grid
        self.over = over
        self.win = win
        self.screen = screen
...
class GameManager(object):
    ...
    @property
    def screen(self):
        return Screen(screen=self.stdscr, grid=self.grid, win=self.win, over=self.over)
    ...
    @property
    def is_over(self):
        dirs = self.action.actions[:4]
        self.over = not any(self.can_move(move) for move in dirs)
        return self.over
    ...
    def _restart_or_exit(self):
        self.screen.draw()
        return 'init' if self.action.get() != Action.EXIT else 'exit'
